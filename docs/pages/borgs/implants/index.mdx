...
Implants are plug-in SAFE modules for BORGs OS. They pair a `ConditionManager` with a `BorgAuth` contract so outside accounts or smart contracts can influence the BORG core under programmable rules. Implants add extra checks or execute transactions when conditions are satisfied.
